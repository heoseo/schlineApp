...
public class StudyroomChatActivity extends AppCompatActivity {
    ...
    public void btnMethod(View v) {
        if (isConnect == false) {   //접속전
            //사용자가 입력한 닉네임을 받는다.
            String nickName = edit1.getText().toString();//★접속자 닉네임으로 교체
    ...
    class ConnectionThread extends Thread {
        ...
        public void run() {
            try {
                // 접속한다.
                final Socket socket = new Socket("192.168.25.47", 9999);
                //final Socket socket = new Socket("192.168.25.47", 9999);
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        thread.start();
                        Log.i(TAG,"보낸 nickName : " + nickName  );
                    }
                });
    ...
    /*
    여러 스레드가 뷰를 수정하기 위해서는 Runnable 인터페이스를
    구현하는 익명구현 객체를 활용해야 비동기식 문제 처리가능
    - 전달받은 메세지를 TextView에 계속 추가 출력
     */
    //메세지 작업처리
            ...
            try {
                while (isRunning) {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            // 텍스트뷰의 객체를 생성
                            TextView tv = new TextView(StudyroomChatActivity.this);
                            tv.setTextColor(Color.BLACK);
                            tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 22);
                            // 메세지의 시작 이름이 내 닉네임과 일치한다면
                            if (msg.startsWith(user_nickname)) {
                                tv.setBackgroundResource(R.drawable.user);
